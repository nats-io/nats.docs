...
| Backoff           | A sequence of durations controlling the redelivery of messages acknowledgment timeout. Overrides `ackWait`. The sequence length must be less than or equal to `MaxDelivery`. E.g., `MaxDelivery=5` `backoff=[5s, 30s, 300s, 3600s, 84000s]` will re-deliver a message 5 times over one day. | 2.7.1   | Yes      |
