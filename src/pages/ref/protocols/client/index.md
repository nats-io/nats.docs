...
The protocol used to communicate between the NATS server and clients is a simple, text-based protocol. The default transport is a TCP/IP socket, however [WebSockets](/architecture/connectivity) are supported as well as a [UNIX domain socket](/architecture/embedded) if the server is embedded within a Go process.

## Structure

### Control Line

Each interaction between the client and server consists of a control line of text, optionally, followed by message content. Most of the protocol messages don't require content, only `PUB`, `HPUB`, `MSG`, and `HMSG` include payloads.

### Field Delimiters

The fields of NATS protocol messages are delimited by spaces or tabs. Multiple, consecutive space or tab characters will be treated as a single field delimiter.

### Message Delimiters

NATS uses a carriage return (`CR`) followed by a line feed (`LF`) (`CR+LF`, `\r`, `0x0D0A`) to terminate a protocol message. This newline sequence is also used to mark the end of the message payload in `PUB`, `HPUB`, `MSG`, and `HMSG` messages.

### Subjects

Subjects are ASCII strings, composed of one or more tokens. A token is restricted to _printing_ alpha-numeric characters. The `.` character is reserved as subject token delimiter. The asterisk `*` is reserved to indicate a _wildcard_ token, and the `>` character is reserved to indicate any number of trailing wildcard tokens.
...
Subject names should be ASCII characters for maximum interoperability. Due to language constraints and performance, some clients may support UTF-8 subject names, as may the server, however no guarantee of non-ASCII character support is provided.
...
```
INFO {["option_name":option_value],...}\r
```
...
- `connect_urls` : An optional list of server urls that a client can connect to (see not below).
