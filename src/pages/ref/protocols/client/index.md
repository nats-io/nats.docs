...
Subjects are ASCII strings, composed of one or more tokens. A token is restricted to *printing* alpha-numeric characters. The `.` character is reserved as subject token delimiter. The asterisk `*` is reserved to indicate a *wildcard* token, and the `>` character is reserved to indicate any number of trailing wildcard tokens.
