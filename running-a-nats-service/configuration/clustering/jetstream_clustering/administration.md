...
There might be a case though where you know a node will never return, and you want to signal to JetStream that the node will not return. This will remove it from the Stream in question and all its Consumers.
...
A server that's to be peer-removed should ideally already be offline, but may be online still in which case the JetStream system will be disabled during runtime. The server should be shut down and not restarted, or if it is, JetStream should be disabled.
