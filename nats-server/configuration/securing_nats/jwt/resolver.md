...
If the server used requires client authentication, or you want to specify which CA is trusted for the lookup of account information, specify `resolver_tls`. This [`tls` configuration map](/nats-server/configuration/securing_nats/tls.md) lets you further restrict TLS to the resolver.
